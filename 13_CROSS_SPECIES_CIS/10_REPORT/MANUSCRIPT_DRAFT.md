...
**A degree-dominated control architecture with a small, universal,
degree-independent residual in the human structural connectome: cross-scale
replication of the fly chokepoint pattern**
...
human structural connectome at 900 subjects (AOMIC-ID1000, SIFT2 streamlines,
456-node parcellation, 15% proportional threshold), using exact per-node CIS
...
1. **Outcome-B replication at a new scale.** The fly study's decisive lesson
   ...
   content — replicates across a 10⁵-fold node-count difference.
...
3. **Why the anatomy diverges.** In the fly, the surviving residual was
   ...
   that the mechanism *selecting* residual chokepoints is not conserved —
   plausibly because it depends on what the reconstruction resolves
...
6. **What would falsify the architectural claim.** If the layer-1
   ...
   the *scale-invariance* framing: it rests on one fly reconstruction and
   one human pipeline, and should be tested on the BANC/larval connectomes
   and on multi-shell human derivatives before being stated as a law.
